...
partial class FlatArrayFlatListTest
{
    ...
    [InlineData(Zero)]
    [InlineData(One)]
    public void IndexerGet_IReadOnlyList_SourceIsEmpty_ExpectIndexOutOfRangeException(int index)
    {
        var source = TestHelper.CreateEmptyFlatListAsReadOnly<RecordType>();
    ...
    public void IndexerGet_IReadOnlyList_IndexIsInRange_ExpectItemIsFromSourceItemsByIndex(
        int index, int sourceLength, params int?[] sourceItems)
    {
        var source = sourceItems.InitializeFlatListAsReadOnly(sourceLength);
    ...
    public void IndexerGet_IReadOnlyList_IndexIsOutOfRange_ExpectIndexOutOfRangeException(
